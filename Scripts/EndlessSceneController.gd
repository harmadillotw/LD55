...
var cross_texture = preload("res://Images/cross.png")
var tick_texture = preload("res://Images/tick.png")
var spawnLocation 
var summonProgress
var text_delay = 4.0
...
var poss_item_array = [1,2,3,4,5,0]
var validation_array =[0,0,0,0,0,0]
var validation_results_array =[0,0,0,0,0,0]
...
func _create_new_validation():
	for n in 6:
		var rand_num = rng.randi_range(0,poss_item_array.size()-1)
		validation_array[n] = poss_item_array[rand_num]
		poss_item_array.remove_at(rand_num)
	print(str(validation_array))
func _on_summon_progress_value_changed(value):
	if value == 100:
		if validate_spawn(validation_array):
			spawn_summon(1)
			Global.successful_summons += 1
			...
			$SuccessfulSummons.text = "Successful Summons\r\n" + str(Global.successful_summons)
			$SuccessPanel/ColorRect/Label.text= str(Global.successful_summons) + "\r\nsuccessful\r\nsummons"
			...
			print(str(validation_results_array))
			$Panel.visible = true
...
func validation_results( spawn_validation):
	var valid = true
	print(str(validation_array))
	print(str(Global.object_at_postion))
	for n in range(1,6):
		...
		#print(str(Global.object_at_postion[n]))
		if  Global.object_at_postion[n] != spawn_validation[n-1]:
			validation_results_array[n-1] = 0
		else:
			validation_results_array[n-1] = 1
	$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect1.texture = cross_texture
	if validation_results_array[0] == 1:
		$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect1.texture = tick_texture

	$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect2.texture = cross_texture
	if validation_results_array[1] == 1:
		$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect2.texture = tick_texture
		
	$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect3.texture = cross_texture	
	if validation_results_array[2] == 1:
		$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect3.texture = tick_texture
		
	$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect4.texture = cross_texture
	if validation_results_array[3] == 1:
		$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect4.texture = tick_texture
		
	$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect5.texture = cross_texture
	if validation_results_array[4] == 1:
		$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect5.texture = tick_texture
		
	$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect6.texture = cross_texture
	if validation_results_array[5] == 1:
		$Panel/ColorRect/HBoxContainer/VBoxContainer2/ResultRect6.texture = tick_texture
...
func _on_v_continue_button_pressed():
	$Panel.visible = false
...
func _on_s_continue_button_pressed():
	await play_sound(click_sound)
	get_tree().change_scene_to_file("res://Scenes/MainEndlessScene.tscn")
